...
#데이터 불러오기
lot <- read.csv("../data/adp/모의고사 R코드 및 데이터/모의고사 1회/lotto.csv")
str(lot)
...
#데이터 불러오기
fifa <- read.csv("../data/adp/모의고사 R코드 및 데이터/모의고사 1회/FIFA.csv")
str(fifa)
head(fifa)
...
#Height 변수의 피트, 인치 단위로 저장된 키 값을 cm 단위의 값으 로 변환하기
...
library(tidyverse)
fifa <- fifa %>%
  mutate(Height_cm = as.numeric(fifa$Height %>% str_split("\\'") %>% sapply(`[`, 1)) * 30 +
      as.numeric(fifa$Height %>% str_split("\\'") %>% sapply(`[`, 2)) * 2.5)
####
